...
def load_models():
    if 'models_loaded' not in st.session_state:
        st.session_state.assists_pipeline = joblib.load("topassists_pipeline_final.pkl")
        st.session_state.league_model = joblib.load("league_winner_model.pkl")
...
# --------------------------
    ...
    seasons = [f"{year}/{str(year+1)[-2:]}" for year in range(1992, 2026)]
    with col1:
    ...
    with col4:
        Club_Total_Goals = st.text_input("Club Total Goals", "60")
        Club_League_Rank = st.text_input("Club League Position", "5")
    ...
    if st.button("🔮 Predict Season Assists", key="predict_assists"):
        ...
        try:
            ...
            Club_Total_Goals_i = int(Club_Total_Goals)
            Club_League_Rank_i = int(Club_League_Rank)
        ...
        input_df = pd.DataFrame({
            ...
            "Club_Total_Goals": [Club_Total_Goals_i],
            "Club_League_Rank": [Club_League_Rank_i],
            "Big6_Club_Feature": [Big6_Club_Feature],
            "Club_Attack_Share": [Club_Attack_Share_f],
            "Club_xG": [Club_xG_f]
        })

        for col in ["Season", "Club", "Position", "Set_Piece_Involvement", "Big6_Club_Feature"]:
        ...
        input_df["Club_Goal_Ratio"] = input_df["Club_Total_Goals"] / input_df["Club_League_Rank"]
