...
def load_models():
    if 'models_loaded' not in st.session_state:
        ...
        # candidates for assists pipeline (check current folder first)
        assists_candidates = [
            script_dir / 'topassistspipeline.pkl',
            script_dir / 'topassists_pipeline.pkl',
            script_dir / 'topassists_pipeline_final.pkl',
            script_dir / 'topassists_pipeline_midseason.pkl'
        ]
        assists_path = next((p for p in assists_candidates if p.exists()), None)
        if assists_path is None:
            st.error(f"Assists pipeline not found. Checked: {[str(p) for p in assists_candidates]}")
            raise FileNotFoundError("Assists pipeline .pkl file not found in repository")

        # candidates for league model
        league_candidates = [
            script_dir / 'league_winner_model.pkl'
        ]
        league_path = next((p for p in league_candidates if p.exists()), None)
        if league_path is None:
            st.error(f"League model not found. Checked: {[str(p) for p in league_candidates]}")
            raise FileNotFoundError("League winner model .pkl file not found in repository")

        st.session_state.assists_pipeline = joblib.load(str(assists_path))
        st.session_state.league_model = joblib.load(str(league_path))
...
# --------------------------
    ...
    # Sample data to help users understand expected inputs
    sample_data = {
        "Matches Played": [38],
        "Wins": [25],
        "Draws": [8],
    ...
    st.markdown("""
    <div style='max-width:900px;margin:12px auto 18px auto;'>
    """, unsafe_allow_html=True)
    ...
    """, unsafe_allow_html=True)
    # use session_state-backed inputs so we can prefill sample values
    input_cols = st.columns(4)
    with input_cols[0]:
        played = st.text_input("Matches Played", value=st.session_state.get('played', "38"), key='played')
    with input_cols[1]:
        won = st.text_input("Wins", value=st.session_state.get('won', "25"), key='won')
    with input_cols[2]:
        drawn = st.text_input("Draws", value=st.session_state.get('drawn', "8"), key='drawn')
    with input_cols[3]:
        lost = st.text_input("Losses", value=st.session_state.get('lost', "5"), key='lost')
    input_cols2 = st.columns(4)
    with input_cols2[0]:
        gf = st.text_input("Goals For", value=st.session_state.get('gf', "80"), key='gf')
    with input_cols2[1]:
        ga = st.text_input("Goals Against", value=st.session_state.get('ga', "30"), key='ga')
    with input_cols2[2]:
        gd = st.text_input("Goal Difference", value=st.session_state.get('gd', "50"), key='gd')
    with input_cols2[3]:
        points = st.text_input("Points", value=st.session_state.get('points', "90"), key='points')
    st.markdown("</div>", unsafe_allow_html=True)
...
# --------------------------
    ...
    with col3:
        ...
        Assists_per_90 = st.text_input("Assists per 90","0.35")
    ...
    if st.button("🔮 Predict Season Assists", key="predict_assists"):
        ...
        try:
            ...
            Assists_per_90_f = float(Assists_per_90)
        except ValueError:
            st.error("Please enter valid numbers for all fields.")
            st.stop()
        input_df = pd.DataFrame({
            ...
            "Assists_per_90": [Assists_per_90_f],
